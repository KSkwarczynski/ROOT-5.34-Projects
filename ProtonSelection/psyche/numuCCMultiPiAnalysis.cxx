...
#include "nueCCUtils.hxx" //WARNING
...
void numuCCMultiPiAnalysis::DefineMicroTrees(bool addBase){
  ...
  AddVarI(   output(), truepi_pdg,      "true pion pdg");
    ...
    AddVarVI(output(), TPCProtonIndex,"",     nProtonsTPC);
    ...
    AddVarVI(output(), TPCProton_ecaldetector,"The detector in which the selected track's ECal segment was found,  DsECal is 6.  Barrel ECal: 7- top, 8-bottom, 9-left, 10-right", nProtonsTPC_ECAL);
    AddVarVI(output(), TPCProton_ecalnhits, "The number of hits in the selected track's ECal segment", nProtonsTPC_ECAL);
    /*
    AddVarVF(output(), FGDProton_ecalemene, "", nProtonsFGD_ECAL);
    AddVarVI(output(), FGDProton_nECALs, "", nProtonsFGD_ECAL);
    AddVarVI(output(), FGDProton_ecaldetector,"", nProtonsFGD_ECAL);
    AddVarVI(output(), FGDProton_ecalnhits, "", nProtonsFGD_ECAL);
    */
    
    //FGD protons variable
    AddVarMF(output(), protFGD_pos," starting pos of proton FGD track", nProtonsFGD,-NMAXTPCSEC,3);
    AddVarVF(output(), FGDProtMom,"",         nProtonsFGD); 
    AddVarVI(output(), FGDProtonTId,"",       nProtonsFGD);
    AddVarVI(output(), FGDProtonIndex,"",     nProtonsFGD);
    AddVarVI(output(), FGDProtonParentTId,"", nProtonsFGD);
    AddVarVI(output(), FGDProtonGParentTId,"",nProtonsFGD);
    
    //True proton Informations
...
void numuCCMultiPiAnalysis::FillMicroTrees(bool addBase){
  ...
  //WARNING
   ...
   if( mybox().MainTrack  ) 
   {
        ...
        Int_t nECAL = anaUtils::GetTrackerDsEcals( mybox().MainTrack, ecaltracks);
        if(nECAL >=1 && nueCCUtils::UseEcal(mybox().MainTrack) )
        {
            ...
            if(EcalSegment && EcalSegment->EMEnergy > 0)
            {
                ...
                output().FillVar(selmu_ecalemene, EcalSegment->EMEnergy);
                output().FillVar(selmu_nECALs, nECAL); //WARNING to mozna poza if()
                output().FillVar(selmu_ecaldetector, detector);   
                output().FillVar(selmu_ecalnhits, EcalSegment->NHits);
                output().FillVar(selmu_ecallength, EcalSegment->Length);
            }
        }
    }
    ...
    for( Int_t i = 0; i < mybox().pionBox.nProtonTPCtracks; i++ )
    {
        ...
        output().FillMatrixVarFromArray(protTPC_pos, track->PositionStart, 3);
        ...
        if( track->GetTrueParticle() )
        {
            ...
            output().FillVectorVar(TPCProtonIndex,     track->GetTrueParticle()->ID); 
            output().FillVectorVar(TPCProtonParentTId, track->GetTrueParticle()->ParentPDG);
            output().FillVectorVar(TPCProtonGParentTId,track->GetTrueParticle()->GParentPDG);
        }
        output().IncrementCounter(nProtonsTPC);
        
        //TODO
        AnaECALParticleB* ecaltracks[NMAXPARTICLES];
        Int_t nECAL = anaUtils::GetTrackerDsEcals(track, ecaltracks);
        if(nECAL >=1 && nueCCUtils::UseEcal(track) )
        {
            ...
            if(EcalSegment && EcalSegment->EMEnergy > 0)
            {
                ...
                output().FillVectorVar(TPCProton_ecalemene, EcalSegment->EMEnergy);
                output().FillVectorVar(TPCProton_nECALs, nECAL); //WARNING to mozna poza if()
                output().FillVectorVar(TPCProton_ecaldetector, detector);   
                output().FillVectorVar(TPCProton_ecalnhits, EcalSegment->NHits);
                
                output().IncrementCounter(nProtonsTPC_ECAL);
            }
        }
    ...
    for( Int_t i = 0; i < mybox().pionBox.nProtonFGDtracks; i++ )
    {
        ...
        output().FillMatrixVarFromArray(protFGD_pos, track->PositionStart, 3);
        output().FillVectorVar(FGDProtMom, track->RangeMomentumProton); //WARNING
        
        if( track->GetTrueParticle() )
        {
            output().FillVectorVar(FGDProtonTId,       track->GetTrueParticle()->PDG);
            output().FillVectorVar(FGDProtonIndex,     track->GetTrueParticle()->ID); 
            output().FillVectorVar(FGDProtonParentTId, track->GetTrueParticle()->ParentPDG);
            output().FillVectorVar(FGDProtonGParentTId,track->GetTrueParticle()->GParentPDG);
        }
        ...
        if(nECAL >=1 && nueCCUtils::UseEcal(track) )
        {
            ...
            if(EcalSegment && EcalSegment->EMEnergy > 0)
            {
                ...
                output().FillVectorVar(FGDProton_ecalemene, EcalSegment->EMEnergy);
                output().FillVectorVar(FGDProton_nECALs, nECAL); //WARNING to mozna poza if()
                output().FillVectorVar(FGDProton_ecaldetector, detector);   
                output().FillVectorVar(FGDProton_ecalnhits, EcalSegment->NHits);
                
                output().IncrementCounter(nProtonsFGD_ECAL);
            }
        }*/
  ...
  //if (_storeRecoPionProtVtx){
    ...
    if (protonCand){
      ...
      output().FillToyVectorVarFromArray(sel_prot_dir_vertex, dir_proton_tmp, 3);
        ...
        //WARNING daj w przyszlosci poprawny kierunek a nie hardcoded
        TVector3 nuDirVec(0, 0, 1);
        costheta_prot_nu = nuDirVec.Dot(dir_proton_tmp);
        output().FillToyVar(sel_prot_costheta, (Float_t)costheta_prot_nu);
    ...
    if (protonCand && vertex){
      ...
      //WARNING      
      output().FillVectorVarFromArray(sel_prot_pos, protonCand->PositionStart, 4);
      output().FillVectorVarFromArray(sel_prot_endpos, protonCand->PositionEnd, 4);

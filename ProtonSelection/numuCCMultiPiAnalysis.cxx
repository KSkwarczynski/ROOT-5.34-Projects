...
void numuCCMultiPiAnalysis::DefineMicroTrees(bool addBase){
  ...
  //TODO
    ...
    //AddVarI(  output(), nProtonsFGD,      "nProtonsFGD");
...
void numuCCMultiPiAnalysis::FillMicroTrees(bool addBase){
  ...
  //WARNING
  ///FILL information about proton
    //int nTPCprotons = 0;
    //int nFGDprotons = 0;
    float CUTvariableTPC=-2000;
    for( Int_t i = 0; i < mybox().pionBox.nProtonTPCtracks; i++ )
    {
        ...
        CUTvariableTPC=track->PositionStart[2];
    }
    
    //std::cout<<"DEBUG KURDE   "<<CUTvariableTPC<<std::endl;
    output().FillVar(CUTprotTPC_pos,   CUTvariableTPC);
    ...
    //output().FillVar(nProtonsTPC, nTPCprotons);
    //output().FillVar(nProtonsFGD, nFGDprotons);

...
// ***************************************************************************
  ...
  if (Sel_Vec.empty()) {
    ...
    Sel_Vec.push_back("FGD1 nu CC0pi-Np"); //WARNING
  ...
  // FGD1 neutrino CC0pi-0p
  if (std::find(Sel_Vec.begin(), Sel_Vec.end(), std::string("FGD1 nu CC0pi")) != Sel_Vec.end()) {
    addSelection(SampleId::kFGD1NuMuCC0Pi);
  }

    // FGD1 neutrino CC0pi-Np
  if (std::find(Sel_Vec.begin(), Sel_Vec.end(), std::string("FGD1 nu CC0pi-Np")) != Sel_Vec.end()) {
    addSelection(SampleId::kFGD1NuMuCC0PiNp); //WARNING
  }
...
// This might require throwing the starting parameters of the Markov Chain to avoid a stationary chain at the first n steps (n might be as large as 10k!)
void samplePDFND2019::setAsimovFakeDataFluctuated(bool CustomReWeight) {
  // ***************************************************************************
  ...
  for (int i = 0; i < SampleId::kNSamples; ++i) {
    ...
    TH2Poly* MCpdf = (TH2Poly*)(getPDF(i))->Clone(temp.c_str());
    for (int ibin = 1; ibin <= MCpdf->GetNumberOfBins(); ibin++){
      MCpdf->SetBinContent(ibin, rand->Poisson((MCpdf->GetBinContent(ibin))));
    }
...
void samplePDFND2019::addSelection( SampleId::SampleEnum selection, ND280KinematicTypes var1, ND280KinematicTypes var2) {
  ...
  if(sampleName=="FGD1 numuCC 0pi Np") //WARNING
  {
      sampleName="FGD1 numuCC 0pi";
  }
...
double samplePDFND2019::CalcXsecWeight_Spline(const int i) {
  ...
  for (int spline = 0; spline < nSplineParams; spline++) {
    ...
    if (xsecw < 0) {
      std::cerr << "Found negative xsecw, event " << i << ", spline " << splineParsNames[spline] << " = " << xsecw << std::endl;
    }
#endif
  } // End the for loop
  return xsecw;
...
// For NIWG 2019, the base model changed from RFG->SF, so BeRPA is no longer relevant. To keep some freedom in Q2, new normalisation parameters were introduced
...
#endif
    } else if(xsecInfo[i].Q2>0.25 && xsecInfo[i].Q2<0.5){
      xsecw *= xs2015->calcReWeight(Q2_norm_5_pos);
#if DEBUG > 0
      EventsPerNorm[Q2_norm_5_pos]++;
#endif
    } else if(xsecInfo[i].Q2>0.5 && xsecInfo[i].Q2<1.0){
      xsecw *= xs2015->calcReWeight(Q2_norm_6_pos);
#if DEBUG > 0
      EventsPerNorm[Q2_norm_6_pos]++;
#endif
    } else if(xsecInfo[i].Q2>1.0){
      xsecw *= xs2015->calcReWeight(Q2_norm_7_pos);
#if DEBUG > 0
      EventsPerNorm[Q2_norm_7_pos]++;
#endif
    }
  }
  ...
  if((xsecInfo[i].mode == kMaCh3_CCMpi || xsecInfo[i].mode == kMaCh3_CCDIS) && (xsecInfo[i].target == kTarget_C || xsecInfo[i].target == kTarget_O) && (xsecInfo[i].species == kNumu || xsecInfo[i].species == kNumu_bar || xsecInfo[i].species == kNue || xsecInfo[i].species == kNue_bar)) {
    if (xsecInfo[i].species > 0) {
    ...
    if (detsimple) {
      ...
      if (ndims[SampleID] == 1) {
        // detector systematics binning is in lepton momentum
          //WARNING kurwa !!!!
            /// \FGD1_FHC_NU
            if(SampleID == SampleId::kFGD1NuMuCC0Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1NuMuCC0Pi, pLep));
            }
            if(SampleID == SampleId::kFGD1NuMuCC0PiNp) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1NuMuCC0Pi, pLep));
            }
            if(SampleID == SampleId::kFGD1NuMuCC1Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1NuMuCC0PiNp, pLep));
            }
            if(SampleID == SampleId::kFGD1NuMuCCOther) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1NuMuCC1Pi, pLep));
            }
            /// \FGD1_RHC_NU_BAR
            if(SampleID == SampleId::kFGD1AntiNuMuCC0Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1AntiNuMuCCNTracks, pLep));
            }
            if(SampleID == SampleId::kFGD1AntiNuMuCC1Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1AntiNuMuCC0Pi, pLep));
            }
            if(SampleID == SampleId::kFGD1AntiNuMuCCOther) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1AntiNuMuCC1Pi, pLep));
            }
            /// \FGD1_RHC_NU
            if(SampleID == SampleId::kFGD1NuMuBkgInAntiNuModeCC0Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1NuMuBkgInAntiNuModeCCNTracks, pLep));
            }
            if(SampleID == SampleId::kFGD1NuMuBkgInAntiNuModeCC1Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1NuMuBkgInAntiNuModeCC0Pi, pLep));
            }
            if(SampleID == SampleId::kFGD1NuMuBkgInAntiNuModeCCOther) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1NuMuBkgInAntiNuModeCC1Pi, pLep));
            }
            
            /// \FGD2_FHC_NU
            if(SampleID == SampleId::kFGD2NuMuCC0Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2NuMuCC, pLep));
            }
            if(SampleID == SampleId::kFGD2NuMuCC1Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2NuMuCC0Pi, pLep));
            }
            if(SampleID == SampleId::kFGD2NuMuCCOther) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2NuMuCC1Pi, pLep));
            }
            /// \FGD2_RHC_NU_BAR
            if(SampleID == SampleId::kFGD2AntiNuMuCC0Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2AntiNuMuCCNTracks, pLep));
            }
            if(SampleID == SampleId::kFGD2AntiNuMuCC1Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2AntiNuMuCC0Pi, pLep));
            }
            if(SampleID == SampleId::kFGD2AntiNuMuCCOther) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2AntiNuMuCC1Pi, pLep));
            }
            /// \FGD2_RHC_NU
            if(SampleID == SampleId::kFGD2NuMuBkgInAntiNuModeCC0Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2NuMuBkgInAntiNuModeCCNTracks, pLep));
            }
            if(SampleID == SampleId::kFGD2NuMuBkgInAntiNuModeCC1Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2NuMuBkgInAntiNuModeCC0Pi, pLep));
            }
            if(SampleID == SampleId::kFGD2NuMuBkgInAntiNuModeCCOther) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2NuMuBkgInAntiNuModeCC1Pi, pLep));
            }
        ...
        // If 2D
            ...
            if(SampleID == SampleId::kFGD1NuMuCC0Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1NuMuCC0Pi, pLep, CosThLep));
            }
            if(SampleID == SampleId::kFGD1NuMuCC0PiNp) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1NuMuCC0Pi, pLep, CosThLep));
            ...
            if(SampleID == SampleId::kFGD1NuMuCC1Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1NuMuCC0PiNp, pLep, CosThLep));
            }
            if(SampleID == SampleId::kFGD1NuMuCCOther) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1NuMuCC1Pi, pLep, CosThLep));
            }
            /// \FGD1_RHC_NU_BAR
            if(SampleID == SampleId::kFGD1AntiNuMuCC0Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1AntiNuMuCCNTracks, pLep, CosThLep));
            }
            if(SampleID == SampleId::kFGD1AntiNuMuCC1Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1AntiNuMuCC0Pi, pLep, CosThLep));
            }
            if(SampleID == SampleId::kFGD1AntiNuMuCCOther) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1AntiNuMuCC1Pi, pLep, CosThLep));
            }
            /// \FGD1_RHC_NU
            if(SampleID == SampleId::kFGD1NuMuBkgInAntiNuModeCC0Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1NuMuBkgInAntiNuModeCCNTracks, pLep, CosThLep));
            }
            if(SampleID == SampleId::kFGD1NuMuBkgInAntiNuModeCC1Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1NuMuBkgInAntiNuModeCC0Pi, pLep, CosThLep));
            }
            if(SampleID == SampleId::kFGD1NuMuBkgInAntiNuModeCCOther) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD1NuMuBkgInAntiNuModeCC1Pi, pLep, CosThLep));
            }
            
            /// \FGD2_FHC_NU
            if(SampleID == SampleId::kFGD2NuMuCC0Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2NuMuCC, pLep, CosThLep));
            }
            if(SampleID == SampleId::kFGD2NuMuCC1Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2NuMuCC0Pi, pLep, CosThLep));
            }
            if(SampleID == SampleId::kFGD2NuMuCCOther) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2NuMuCC1Pi, pLep, CosThLep));
            }
            /// \FGD2_RHC_NU_BAR
            if(SampleID == SampleId::kFGD2AntiNuMuCC0Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2AntiNuMuCCNTracks, pLep, CosThLep));
            }
            if(SampleID == SampleId::kFGD2AntiNuMuCC1Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2AntiNuMuCC0Pi, pLep, CosThLep));
            }
            if(SampleID == SampleId::kFGD2AntiNuMuCCOther) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2AntiNuMuCC1Pi, pLep, CosThLep));
            }
            /// \FGD2_RHC_NU
            if(SampleID == SampleId::kFGD2NuMuBkgInAntiNuModeCC0Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2NuMuBkgInAntiNuModeCCNTracks, pLep, CosThLep));
            }
            if(SampleID == SampleId::kFGD2NuMuBkgInAntiNuModeCC1Pi) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2NuMuBkgInAntiNuModeCC0Pi, pLep, CosThLep));
            }
            if(SampleID == SampleId::kFGD2NuMuBkgInAntiNuModeCCOther) 
            { 
                ndobj->det_bin.push_back(detsimple->getBin(SampleId::kFGD2NuMuBkgInAntiNuModeCC1Pi, pLep, CosThLep));
            }
...
void samplePDFND2019::FindNormBins() {
  ...
  if (xsecmodel == kNIWG_2020a) {
    ...
    for (std::vector<int>::iterator it = funcParsIndex.begin(); it != funcParsIndex.end(); ++it, ++berpa_it) {
      ...
      if (name == "Q2_norm_0") {
        ...
        Q2_norm_4_pos = *it;
      } else if (name == "Q2_norm_5") {
        Q2_norm_5_pos = *it;
      } else if (name == "Q2_norm_6") {
        Q2_norm_6_pos = *it;
      } else if (name == "Q2_norm_7") {
        Q2_norm_7_pos = *it;
    ...
    if (mec_norm_nu_pos == -999) {
      ...
      std::cerr << "Q2 norm 2 pos == -999" << std::endl;
      throw;
    ...
    } else if (Q2_norm_5_pos == -999) {
      std::cerr << "Q2 norm 5 pos == -999" << std::endl;
      throw;
      
    } else if (Q2_norm_6_pos == -999) {
      std::cerr << "Q2 norm 6 pos == -999" << std::endl;
      throw;
      
    } else if (Q2_norm_7_pos == -999) {
      std::cerr << "Q2 norm 7 pos == -999" << std::endl;
      throw;
  ...
  std::cout << std::left << std::setw(35) << "Q2 norm 5" << Q2_norm_5_pos << std::endl;
  std::cout << std::left << std::setw(35) << "Q2 norm 6" << Q2_norm_6_pos << std::endl;
  std::cout << std::left << std::setw(35) << "Q2 norm 7" << Q2_norm_7_pos << std::endl;

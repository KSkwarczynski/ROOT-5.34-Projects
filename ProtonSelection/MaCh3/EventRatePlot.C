void EvenetRatePlot(TString fname) {
   ...
   while ((key = (TKey*)next())) 
   {
      ...
      TH2Poly *h = (TH2Poly*)key->ReadObj();
      string name;
      name = h->GetName();

      if (name.find("FGD1_numuCC_0pi_nom") != string::npos)
      {
           h->SetTitle("FGD1_numuCC_0pi_0p_nom");
      }
      if (name.find("data") != string::npos)
      {
           continue;
      }
      if (name.find("W2nom") != string::npos)
      {
           continue;
      }
      ...
      /*
      double binCounter=0;
      double BinAll = h->GetNumberOfBins();
      
      for(int i=0; i<BinAll; i++)
      {
        binCounter = h->GetBinContent(i);
        if(binCounter>=20)
        {
            h->SetBinContent(i,0); 
        }

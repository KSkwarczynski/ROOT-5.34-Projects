...
void NEUT_NuWro() {
  ...
  for (int fileIndex = 0; fileIndex < 2; ++fileIndex) {
    ...
    hNuMuPi_Q2[fileIndex] = new TH1D(Form("hNuMuPi_Q2_%s", legendEntries[fileIndex]), "", NBins, 0, 2000);
    hNuEPi_Q2[fileIndex] = new TH1D(Form("hNuEPi_Q2_%s", legendEntries[fileIndex]), "hNuMuPi0_Plep", NBins, 0, 2000);
    ...
    hNuMuPi_Q2[fileIndex]->GetXaxis()->SetTitle("Q^{2} [MeV]");
    hNuMuPi_Q2[fileIndex]->GetYaxis()->SetTitle("A.U.");
    ...
    for (Long64_t i = 0; i < nentries; ++i) {
      ...
      if (PDG == 14) {
        if (mode == 11 || mode == 12 || mode == 13) {
          ...
          hNuMuPi_Q2[fileIndex]->Fill(Q2 * 1000, fScaleFactor);
        }
      ...
      if (PDG == 12) {
        if (mode == 11 || mode == 12 || mode == 13) {
          ...
          hNuEPi_Q2[fileIndex]->Fill(Q2 * 1000, fScaleFactor);
        }
    ...
    if(PlotShape)
    {
      ...
      hNuMuPi[fileIndex]->Scale(1.0 / hNuMuPi[fileIndex]->Integral());
      hNuEPi[fileIndex]->Scale(1.0 / hNuEPi[fileIndex]->Integral());
      hNuMuPi_W[fileIndex]->Scale(1.0 / hNuMuPi_W[fileIndex]->Integral());
      hNuEPi_W[fileIndex]->Scale(1.0 / hNuEPi_W[fileIndex]->Integral());
      ScaleHistogram(hNuMuPi_Plep[fileIndex]);
      ScaleHistogram(hNuEPi_Plep[fileIndex]);
      ScaleHistogram(hNuMuPi_Q2[fileIndex]);
      ScaleHistogram(hNuEPi_Q2[fileIndex]);

      hNuMuPipm[fileIndex]->Scale(1.0 / hNuMuPipm[fileIndex]->Integral());
      hNuMuPi0[fileIndex]->Scale(1.0 / hNuMuPi0[fileIndex]->Integral());
      hNuEPipm[fileIndex]->Scale(1.0 / hNuEPipm[fileIndex]->Integral());
      hNuEPi0[fileIndex]->Scale(1.0 / hNuEPi0[fileIndex]->Integral());

      hNuMuPipm_W[fileIndex]->Scale(1.0 / hNuMuPipm_W[fileIndex]->Integral());
      hNuMuPi0_W[fileIndex]->Scale(1.0 / hNuMuPi0_W[fileIndex]->Integral());
      hNuEPipm_W[fileIndex]->Scale(1.0 / hNuEPipm_W[fileIndex]->Integral());
      hNuEPi0_W[fileIndex]->Scale(1.0 / hNuEPi0_W[fileIndex]->Integral());
      ...
      hNuMuBarPi[fileIndex]->Scale(1.0 / hNuMuBarPi[fileIndex]->Integral());
      hNuEBarPi[fileIndex]->Scale(1.0 / hNuEBarPi[fileIndex]->Integral());
      hNuMuBarPi_W[fileIndex]->Scale(1.0 / hNuMuBarPi_W[fileIndex]->Integral());
      hNuEBarPi_W[fileIndex]->Scale(1.0 / hNuEBarPi_W[fileIndex]->Integral());
      ScaleHistogram(hNuMuBarPi_Plep[fileIndex]);
      ScaleHistogram(hNuEBarPi_Plep[fileIndex]);

      hNuMuBarPipm[fileIndex]->Scale(1.0 / hNuMuBarPipm[fileIndex]->Integral());
      hNuMuBarPi0[fileIndex]->Scale(1.0 / hNuMuBarPi0[fileIndex]->Integral());
      hNuEBarPipm[fileIndex]->Scale(1.0 / hNuEBarPipm[fileIndex]->Integral());
      hNuEBarPi0[fileIndex]->Scale(1.0 / hNuEBarPi0[fileIndex]->Integral());

      hNuMuBarPipm_W[fileIndex]->Scale(1.0 / hNuMuBarPipm_W[fileIndex]->Integral());
      hNuMuBarPi0_W[fileIndex]->Scale(1.0 / hNuMuBarPi0_W[fileIndex]->Integral());
      hNuEBarPipm_W[fileIndex]->Scale(1.0 / hNuEBarPipm_W[fileIndex]->Integral());
      hNuEBarPi0_W[fileIndex]->Scale(1.0 / hNuEBarPi0_W[fileIndex]->Integral());
    ...
    NiceHistogram(hNuMuPi_Q2[fileIndex], Style[fileIndex], kBlue);
    NiceHistogram(hNuEPi_Q2[fileIndex], Style[fileIndex], kRed);
  ...
  if(hNuEPi_Q2[1]->GetMaximum() > hNuMuPi_Q2[0]->GetMaximum()) hNuMuPi_Q2[0]->SetMaximum(hNuEPi_Q2[1]->GetMaximum());
  hNuMuPi_Q2[0]->Draw("hist");
  hNuEPi_Q2[0]->Draw("hist SAME");
  hNuMuPi_Q2[1]->Draw("hist SAME");
  hNuEPi_Q2[1]->Draw("hist SAME");

  TLegend *Legend_Tot_Q2 = new TLegend(0.6, 0.6, 0.9, 0.9);
  Legend_Tot_Q2->SetFillColor(0);
  Legend_Tot_Q2->SetFillStyle(0);
  Legend_Tot_Q2->SetLineWidth(0);
  Legend_Tot_Q2->SetLineColor(0);
  Legend_Tot_Q2->AddEntry(hNuMuPi_Q2[0], "NEUT #nu_{#mu} CCSPP", "l");
  Legend_Tot_Q2->AddEntry(hNuEPi_Q2[0], "NEUT #nu_{e} CCSPP", "l");
  Legend_Tot_Q2->AddEntry(hNuMuPi_Q2[1], "NUWRO #nu_{#mu} CCSPP", "l");
  Legend_Tot_Q2->AddEntry(hNuEPi_Q2[1], "NUWRO #nu_{e} CCSPP", "l");
  Legend_Tot_Q2->Draw("SAME");

  c1->Print("NIWG_Plot.pdf");
  delete Legend_Tot_Q2;

...
void Binning(std::string filename, std::string binPoly) {
   ...
   std::string dupa = filename + binPoly;
   ...
   while ((key = (TKey*)next())) 
   {
      ...
      if (name.find("_W2Hist") != std::string::npos) continue;
        ...
        for (int i = 1; i < poly->GetNumberOfBins()+1; ++i) 
        {
            ...
            double ymas = bin->GetYMax() - 0.005;
            double xup = bin->GetXMax() - 25;
            FluctHist->SetBinContent(FluctHist->FindBin(xup, ymas), FluctHist->GetBinContent(i) + bin->GetContent());
